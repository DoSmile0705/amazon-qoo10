...
const addProductToMydb = async (req, res) => {
  try {
    ...
    let product = await Product.findOne({ asin: asin, userId: req.body.userId });
    if (product) {
    ...
    product = new Product({
      ...
      description: catalog_item.attributes.product_description ? catalog_item.attributes.product_description[0].value : bullet_point,
      ...
      recommended_browse_nodes: catalog_item.attributes.recommended_browse_nodes[0],
      ...
      websiteDisplayGroupName: catalog_item.summaries[0].websiteDisplayGroupName,
...
const updateProductOfMydb = async (req, res) => {
  ...
  Product.updateOne({ asin: req.body.asin, userId: req.body.userId }, { $set: req.body })
    .then((product) => res.json({ product: product, message: '商品情報変更成功' }))
    .catch((err) => res.status(404).json({ nopostfound: "No Products found" }));
};
const updateProductOfMydbRelatedToTime = async () => {
  ...
  if (products.length) {
    products.map(async (product, index) => {
      ...
      const qoo10_price = price + product.odds_amount + price * product.bene_rate / 100
      Product.updateOne({ asin: asin }, { $set: { price: price, qoo10_price: qoo10_price } })
        .then((product) => {
        })
        .catch((err) => {
          console.log(err);
  ...
  if (data.length) {
    if (ngData.length) {
      await NgData.updateOne(
        ...
        .then((products) => res.status(200).json({ message: 'ngword successfully inserted' }))
        .catch((err) => res.status(404).json({ nopostfound: "No Products found" }));
    } else {
      ngData = new NgData({
        ngword: undefined,
