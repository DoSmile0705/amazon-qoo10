...
const addNgData = async (req, res) => {
  ...
  try {
    ...
    const is_exist_ngword = await NgData.find({ "_id": reqData.userId, 'ngword.value': reqData.ngword?.value });
    const is_exist_ngbrand = await NgData.find({ "_id": reqData.userId, 'ngbrand.value': reqData.ngbrand?.value });
    const is_exist_ngcategory = await NgData.find({ "_id": reqData.userId, 'ngcategory.value': reqData.ngcategory?.value });
    const is_exist_ngasin = await NgData.find({ "_id": reqData.userId, 'ngasin.value': reqData.ngasin?.value });
    const is_exist_excludeword = await NgData.find({ "_id": reqData.userId, 'excludeword.value': reqData.excludeword?.value });
    if (!ngdata.length) {
      ngdata = new NgData({
        _id:reqData.userId,
      ...
      NgData.updateOne(
        { _id: reqData.userId },
        {
          $push: {
            ...
            ngcategory: !is_exist_ngcategory.length ? reqData.ngcategory : undefined,
            ngasin: !is_exist_ngasin.length ? reqData.ngasin : undefined,
            excludeword: !is_exist_excludeword.length ? reqData.excludeword : undefined,
          },
        }
      )
...
const useNgData = async (req, res) => {
  ...
  await NgData.findOneAndUpdate(
    {
      "_id": userId, [[kind] + ".value"]: value
    },
    {
      $set: { [[kind] + '.$.flag']: flag, [[kind] + '.$.value']: editedValue },
    }
  ).then((ngdata) => res.status(200).json({ ngdata: ngdata, message: 'ngword successfully updated' }))
...
const deleteNgData = async (req, res) => {
  ...
  ).then((ngdata) => res.status(200).json({ ngdata: ngdata, message: 'ngword successfully deleted' }))
    .catch((err) => res.status(404).json({ nopostfound: "No ngword found" }));
