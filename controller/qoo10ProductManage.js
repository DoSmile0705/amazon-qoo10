...
const createCategory = async () => {
  ...
  if (qoo10categorys.length === 0) {
    ...
    Category.insertMany(documents).then(result => {
    }).catch(err => {
...
const CreateCertificationKey = async () => {
  const url = 'https://api.qoo10.jp/GMKT.INC.Front.QAPIService/CertificationAPI.qapi/CreateCertificationKey';
  const requestConfig = {
    headers: {
...
const getQoo10Category = async (req, res) => {
  Category.find()
    ...
    .catch((err) => res.status(404).json({ nopostfound: "No categories found" }));
};
const setNewGoods = async (req, res) => {
  const certificationKey = await CreateCertificationKey();
  let flag = false;
  req.body.map((good, index) => {
    const url = 'https://api.qoo10.jp/GMKT.INC.Front.QAPIService/ItemsBasic.qapi/SetNewGoods';
    const requestConfig = {
      headers: {
      ...
      .then(async (response) => {
        if (!response.data.ResultCode) {
          ...
          Product.findOneAndUpdate(
            ...
            {
              status: good.status,
              ItemCode: response.data.ResultObject.GdNo
            },
          )
            .then(async (products) => {
              const data = await Product.find({ status: '新規追加', userId: good.userId });
              ...
              await Product.deleteMany({ status: '新規追加', userId: good.userId })
            })
            .catch((err) => {console.log(err);});
            if(!flag)
         { res.status(200).json({ message: 'qoo10に正確に出品されました。', status: true });
            flag=true;
          }
        } 
      })
      .catch(error => {
        res.status(404).json({ message: "qoo10エロへの商品提出に失敗しました。" });
      });
  });
}
const updatePrice = async (ItemCode, qoo10_price) => {
  const certificationKey = await CreateCertificationKey();
  const url = 'https://api.qoo10.jp/GMKT.INC.Front.QAPIService/ItemsOrder.qapi/SetGoodsPriceQty';
  const requestConfig = {
    headers: {
      'Giosiscertificationkey': certificationKey,
    },
    ...
    .then(async (response) => {
      if (response) {
        await Product.findOneAndUpdate(
          { ItemCode: ItemCode },
          {
            selledQuantity: (last_quantity - response.data.ResultObject[0]?.ItemQty),
